#!/usr/bin/env python3
"""Evaluate one measured PRT run at any validated sample count."""
...
def main() -> None:
    ...
    parser.add_argument("--paired-reference-root", "--reference-root")
    ...
    reference_root = (
        Path(args.paired_reference_root).resolve(strict=True)
        if args.paired_reference_root
        else None
    )
    manifest_path = Path(args.manifest).resolve(strict=True)
    reference_manifest_path = (
        Path(args.reference_manifest).resolve(strict=True)
        if args.reference_manifest
        else None
    )
    if (reference_root is None) != (reference_manifest_path is None):
        raise ValueError("paired reference root and manifest must be supplied together")
    records = load_manifest(manifest_path)
    expected_per_class = (
        args.expected_count // 1000 if args.expected_count % 1000 == 0 else None
    )
    validate_manifest(
        records,
        expected_count=args.expected_count,
        expected_per_class=expected_per_class,
        expected_num_classes=1000 if expected_per_class is not None else None,
    )
    ...
    if reference_run is not None and reference_manifest_path is not None:
    ...
    if trace["sample_count"] != args.expected_count:
        raise ValueError(
            f"trace covers {trace['sample_count']} samples, expected {args.expected_count}"
        )
    ...
    paired = None
    pair_rows = []
    if reference_root is not None and reference_manifest_path is not None:
    ...
    baseline = _baseline_row(Path(args.baseline_summary), args.model) if args.baseline_summary else None
    if baseline is not None and reference_root is not None:
    ...
    if (
        ...
        or int(timing.get("cumulative_sample_count", -1)) != args.expected_count
    ...
    row: dict[str, object] = {
        ...
        "sample_count": args.expected_count, "elapsed_seconds": elapsed,
        "images_per_second": args.expected_count / elapsed,
        "speedup_vs_full": (
            float(baseline["elapsed_seconds"]) / elapsed if baseline is not None else ""
        ),
