...
def register_trajectory(
    ...
    """Accept only identical per-sample copies of one manifest batch summary."""
    ...
    report: dict[str, Any] = {
        ...
        **{key: next(iter(values)) for key, values in identities.items()},
        ...
        "note": (
            ...
            "host perf_counter components surround asynchronous CUDA work and are not "
            "an exact CUDA decomposition; use the CUDA-event latency report for total "
            "sampling time"
        ),
    }
