...
from .metadata import SPECA_CONFIG_FIELDS
...
def resumable_batch_groups(
    ...
    *,
    ...
    speca_config: Mapping[str, object] | None = None,
    checkpoint_path: str,
    checkpoint_size: int,
    method: str,
    interval: int | None,
    ...
    for group_id, values in sorted(
        ...
        for value in values:
            ...
            if speca_config is not None:
                missing = [field for field in SPECA_CONFIG_FIELDS if field not in speca_config]
                if missing:
                    raise ValueError(f"resume SpeCa config is missing fields: {missing}")
                checks.update(
                    {field: speca_config[field] for field in SPECA_CONFIG_FIELDS}
                )
