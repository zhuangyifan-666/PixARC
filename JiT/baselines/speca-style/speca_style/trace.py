...
from .error_metrics import error_value
...
def relative_errors(exact: torch.Tensor, forecast: torch.Tensor) -> dict[str, float]:
    ...
    return {
        ...
        "relative_l1": error_value(forecast.float(), exact.float(), metric="relative_l1"),
        "relative_l2": error_value(forecast.float(), exact.float(), metric="relative_l2"),
        "cosine_error": error_value(
            forecast.float(), exact.float(), metric="cosine_similarity"
        ),
    }
