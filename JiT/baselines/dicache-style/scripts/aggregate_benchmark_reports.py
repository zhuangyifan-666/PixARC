#!/usr/bin/env python3
"""Aggregate repeated matched latency/memory JSON reports on CPU."""
    ...
    for role in ("full", "dicache"):
        ...
        combined[role] = {
            **summarize_latencies(raw),
            ...
            "guard_recompile_count_sum": sum(
                int(report[role].get("guard_recompile_count", 0))
                for report in reports
            ),
        }
    full_median = float(combined["full"]["median_ms_per_image"])
    dicache_median = float(combined["dicache"]["median_ms_per_image"])
    combined.update(
        {
            ...
            "protocol": protocol,
            "median_speedup": full_median / dicache_median,
