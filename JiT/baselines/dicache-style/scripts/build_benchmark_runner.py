#!/usr/bin/env python3
"""Build a manifest-bound JiT benchmark-runner JSON without touching CUDA."""
...
def main() -> None:
    ...
    records = load_manifest(Path(args.manifest).resolve(strict=True))
    first_group = records[0].batch_group_id
    group = [record for record in records if record.batch_group_id == first_group]
    group.sort(key=lambda record: record.position_in_batch)
