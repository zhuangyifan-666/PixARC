...
def _json_safe_diagnostic(value: object) -> object:
    """Detach durable diagnostics from non-finite Python/NumPy scalars.

    A non-finite diagnostic is represented as JSON ``null``.  This sanitizer
    deliberately runs after the gate/DCTA decision and therefore cannot alter
    executable cache semantics.
    """
...
def _event_weighted_mean(
    ...
    *,
    ...
    """Aggregate event means without giving short trajectories extra weight.

    New summaries carry exact sums and counts.  The mean-times-count fallback
    keeps old diagnostic summaries readable without pretending their missing
...
def _shadow_diagnostics(events: Sequence[Mapping[str, object]]) -> dict[str, object]:
    def report(subset: Sequence[Mapping[str, object]]) -> dict[str, object]:
        probe: list[float] = []
        full: list[float] = []
        zero: list[float] = []
        dcta: list[float] = []
        ...
        gamma_clip_max = 0
        for event in subset:
            probe_value = _finite_number(event.get("delta_y"))
            full_value = _finite_number(event.get("actual_full_residual_change"))
            if probe_value is not None and full_value is not None:
                probe.append(probe_value)
                full.append(full_value)
            zero_value = _finite_number(event.get("zero_order_relative_error"))
            dcta_value = _finite_number(event.get("dcta_relative_error"))
            if zero_value is not None and dcta_value is not None:
                zero.append(zero_value)
                dcta.append(dcta_value)
            gamma_value = _finite_number(event.get("gamma"))
            gamma_raw_value = _finite_number(event.get("gamma_raw"))
            if gamma_value is not None:
                gamma.append(gamma_value)
            if gamma_raw_value is not None:
                gamma_raw.append(gamma_raw_value)
            dcta_used += int(bool(event.get("dcta_used", False)))
            zero_fallback += int(bool(event.get("zero_order_fallback", False)))
            gamma_clip_max += int(bool(event.get("gamma_clipped_max", False)))
        zero_mean = sum(zero) / len(zero) if zero else None
        dcta_mean = sum(dcta) / len(dcta) if dcta else None
        return {
            "event_count": len(subset),
            "probe_full_pair_count": len(probe),
            "probe_full_spearman": _spearman(probe, full),
            "reuse_error_pair_count": len(zero),
            ...
            "dcta_better_fraction": (
                sum(new < old for old, new in zip(zero, dcta, strict=True)) / len(zero)
                if zero
                else None
            ),
            "gamma_count": len(gamma),
            "gamma_mean": sum(gamma) / len(gamma) if gamma else None,
            "gamma_p95": percentile(gamma, 95) if gamma else None,
            "gamma_raw_mean": sum(gamma_raw) / len(gamma_raw) if gamma_raw else None,
            "dcta_used_count": dcta_used,
            "zero_order_fallback_count": zero_fallback,
            "gamma_max_clip_rate": (
                gamma_clip_max / dcta_used if dcta_used else 0.0
            ),
        }
    ...
    for event in events:
        stages.setdefault(str(event.get("solver_stage", "unknown")), []).append(event)
    result["by_solver_stage"] = {
        stage: report(subset) for stage, subset in sorted(stages.items())
    }
    return result
