...
class DiCacheRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        if profile not in {"flux_image_released", "explicit_ablation"}:
            raise ValueError("unsupported executable profile")
        if probe_depth < 1:
        ...
        if gamma_min > gamma_max:
            raise ValueError("gamma_min cannot exceed gamma_max")
        ...
        if warmup_semantics not in {"flux_inclusive", "exact_count_ablation"}:
            raise ValueError("unsupported warmup_semantics")
        if profile == "flux_image_released" and mode in {
            "dicache", "dicache_zero_order", "probe_shadow_full"
        }:
            fixed = {
                "error_choice": (error_choice, "delta_y"),
                "ret_ratio": (ret_ratio, 0.2),
                "gamma_min": (gamma_min, 1.0),
                "gamma_max": (gamma_max, 1.5),
                "force_last_full": (force_last_full, True),
                "numeric_mode": (numeric_mode, "official_no_epsilon"),
                "warmup_semantics": (warmup_semantics, "flux_inclusive"),
            }
            ...
            mismatch = {name: pair for name, pair in fixed.items() if pair[0] != pair[1]}
            if mismatch:
                raise ValueError(f"flux_image_released fixed field mismatch: {mismatch}")
        self.mode = mode
        ...
        self.warmup_semantics = warmup_semantics
        ...
        self._shadow_payloads: dict[Hashable, dict[str, object]] = {}
    ...
    def observe_probe(
        ...
        *,
        ...
        gated = strict_accumulated_gate(state.accumulated_error, measured.error, threshold)
        scalar_sync_time_ms = (
        ...
        if self.mode == "probe_shadow_full":
            hypothetical = action
            action = FULL_RESUME_FROM_PROBE
            reason = "shadow_exact_execution"
        elif self.mode == "probe_only_ablation":
            hypothetical = action
            action = FULL_RESUME_FROM_PROBE
            state.accumulated_error = 0.0
            reason = "probe_only_ablation"
        ...
        # Diagnostics must never make the durable JSON contract emit NaN/Inf.
        # The tensors above still drive the exact released gate semantics; only
        # the detached scalar summaries are filtered here.
        ...
        accumulated_scalar = _finite_scalar(gated.accumulator_before + measured.error)
        if accumulated_scalar is not None:
            state.accumulated_values.append(accumulated_scalar)
        gate_elapsed_ms = (time.perf_counter() - gate_started) * 1000.0
        state.scalar_sync_time_ms += scalar_sync_time_ms
        state.gate_time_ms += max(0.0, gate_elapsed_ms - scalar_sync_time_ms)
        return decision
    ...
    def estimate_reuse(
        ...
        *,
        ...
        dcta_started = time.perf_counter()
        try:
            ...
            scalar_sync_time_ms = error.scalar_sync_time_ms
            elapsed_ms = (time.perf_counter() - dcta_started) * 1000.0
            state.scalar_sync_time_ms += scalar_sync_time_ms
            state.dcta_time_ms += max(0.0, elapsed_ms - scalar_sync_time_ms)
            raise
        ...
        elapsed_ms = (time.perf_counter() - dcta_started) * 1000.0
        state.scalar_sync_time_ms += result.scalar_sync_time_ms
        state.dcta_time_ms += max(0.0, elapsed_ms - result.scalar_sync_time_ms)
        return result
    ...
    def add_component_time(
        ...
        *,
        ...
        """Add model-side timings; host timings are diagnostic for async CUDA."""

        _, state = self._require_open_stream(stream_id)
        for name, value in (
            ("probe_time_ms", probe_time_ms),
            ("suffix_time_ms", suffix_time_ms),
            ("scalar_sync_time_ms", scalar_sync_time_ms),
        ):
            if value < 0:
                raise ValueError(f"{name} cannot be negative")
            setattr(state, name, getattr(state, name) + float(value))
    ...
    def complete_full(
        ...
        *,
        ...
        if self.mode != "instrumented_full":
            ...
            if self.mode == "probe_shadow_full":
                ...
                if previous_actual is not None:
                    ...
                    actual_change = _finite_scalar(
                        (full_residual - previous_actual).abs().mean() / divisor
                    )
                self._shadow_previous_actual_residuals[plan.stream_id] = (
                    full_residual.detach().clone()
                )
                self._shadow_payloads[plan.stream_id] = {
                    "actual_full_residual_change": actual_change
                }
            decision = self._pending_decisions.get(plan.stream_id)
            hypothetical_reuse = (
                self.mode == "probe_shadow_full"
                and decision is not None
                and decision.hypothetical_action == REUSE
            )
            if hypothetical_reuse:
                latest = state.anchors.latest.full_residual
                zero_error = (latest - full_residual).abs().mean() / full_residual.abs().mean()
                shadow: dict[str, object] = {
                    "hypothetical_action": REUSE,
                    "zero_order_relative_error": _finite_scalar(zero_error),
                }
                try:
                    prediction = estimate_residual(
                        body_input,
                        probe_feature,
                        state.anchors,
                        gamma_min=self.gamma_min,
                    ...
                    shadow["dcta_force_full"] = True
                    shadow["dcta_error"] = type(error).__name__
                    shadow["counterfactual_effective_action"] = FULL_RESUME_FROM_PROBE
                    shadow["gamma_raw"] = None
                    shadow["gamma"] = None
                    shadow["gamma_clipped_min"] = False
                    shadow["gamma_clipped_max"] = False
                    shadow["gamma_nonfinite"] = True
                    shadow["dcta_used"] = False
                    shadow["zero_order_fallback"] = False
                    state.anchors.append_exact(
                        full_residual=full_residual.detach(), probe_residual=probe_residual.detach(),
                        nfe_index=trajectory.nfe_index, stream_call_index=state.call_index,
                        continuous_t=trajectory.continuous_t, solver_stage=trajectory.solver_stage,
                    )
                    state.accumulated_error = 0.0
                else:
                    dcta_error = (
                        prediction.estimated_residual - full_residual
                    ).abs().mean() / full_residual.abs().mean()
                    shadow.update(
                        {
                            "dcta_force_full": False,
                            "dcta_relative_error": _finite_scalar(dcta_error),
                            "gamma_raw": _finite_scalar(prediction.gamma_raw),
                            "gamma": _finite_scalar(prediction.gamma),
                            "gamma_clipped_min": prediction.gamma_clipped_min,
                            "gamma_clipped_max": prediction.gamma_clipped_max,
                            "gamma_nonfinite": prediction.gamma_nonfinite,
                            "dcta_used": prediction.dcta_used,
                            "zero_order_fallback": prediction.zero_order_fallback,
                        }
                    )
                self._shadow_payloads.setdefault(plan.stream_id, {}).update(shadow)
            else:
                state.anchors.append_exact(
        ...
        if resumed:
            ...
            action = DIRECT_FULL
        state.cache_io_time_ms += (time.perf_counter() - cache_started) * 1000.0
    ...
    def complete_reuse(
        ...
        *,
        ...
        cache_started = time.perf_counter()
        _, state = self._require_open_stream(decision.stream_id)
        self._validate_body_triplet(body_input, probe_feature, result.approximated_body_output)
    ...
    def end_trajectory(self, *, require_complete: bool = True, reset: bool = True) -> dict[str, object]:
        ...
        accumulated = [value for state in states for value in state.accumulated_values]
        ...
        summary: dict[str, object] = {
            ...
            "accumulated_error_nonfinite_count": probe_count - len(accumulated),
            ...
            "mean_accumulated_error": sum(accumulated) / len(accumulated) if accumulated else 0.0,
            "accumulated_error_value_count": len(accumulated),
            "accumulated_error_value_sum": sum(accumulated),
            "p95_accumulated_error": percentile(accumulated, 95),
            "max_accumulated_error": max(accumulated, default=0.0),
            ...
            "probe_time_ms": sum(state.probe_time_ms for state in states),
            "gate_time_ms": sum(state.gate_time_ms for state in states),
            "scalar_sync_time_ms": sum(state.scalar_sync_time_ms for state in states),
            "dcta_time_ms": sum(state.dcta_time_ms for state in states),
            "suffix_time_ms": sum(state.suffix_time_ms for state in states),
            "cache_io_time_ms": sum(state.cache_io_time_ms for state in states),
            "scalar_sync_accounting": "included_in_gate_or_dcta_host_elapsed_unless_explicitly_measured",
    ...
    def _finish_stream(
        ...
        *,
        ...
        # Summary/off modes must not construct a per-call dictionary: doing so
        # would still call ``Tensor.item()`` and serialize scalar diagnostics on
        # every 50K-generation call even though TraceCollector discards them.
        if self.trace.mode in {"full", "shadow"}:
            event = {
                "nfe_index": trajectory.nfe_index,
                ...
                "gamma_raw": _finite_scalar(result.gamma_raw) if result else None,
                "gamma": _finite_scalar(result.gamma) if result else None,
                "gamma_clipped_min": result.gamma_clipped_min if result else False,
                "gamma_clipped_max": result.gamma_clipped_max if result else False,
                "dcta_used": result.dcta_used if result else False,
                "zero_order_fallback": result.zero_order_fallback if result else False,
                "resume_used": action == FULL_RESUME_FROM_PROBE,
                "hypothetical_action": decision.hypothetical_action if decision else None,
                "t_next": trajectory.t_next,
                "probe_error_finite": decision.probe_error.finite if decision else True,
                "delta_x_denominator": _finite_scalar(decision.probe_error.delta_x_denominator) if decision else None,
                "delta_y_denominator": _finite_scalar(decision.probe_error.delta_y_denominator) if decision else None,
            }
            event.update(self._shadow_payloads.get(stream_id, {}))
