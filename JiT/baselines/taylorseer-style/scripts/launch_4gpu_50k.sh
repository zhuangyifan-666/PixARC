...
for REQUIRED in "$GENERATE_SHARD" "$SNAPSHOT_SCRIPT" "$WALL_CLOCK_SCRIPT"; do
...
if [[ "${TAYLORSEER_GPU_TESTS_ALLOWED:-0}" != "1" ]]; then
  echo "Refusing GPU generation. Set TAYLORSEER_GPU_TESTS_ALLOWED=1 only after all target GPUs are idle." >&2
  exit 2
fi
...
LOCK_DIR="$OUTPUT_ROOT/.taylorseer-launch.lock"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "Refusing launch: another TaylorSeer launcher owns $LOCK_DIR." >&2
  exit 4
fi
...
GPU_LOCK_ROOT="/tmp/pixarc-taylorseer-gpu-locks"
...
for DEVICE_UUID in "${PHYSICAL_GPU_UUIDS[@]}"; do
  ...
  if ! mkdir "$GPU_LOCK_DIR" 2>/dev/null; then
    ...
    echo "Verify no TaylorSeer rank is active before removing a stale lock." >&2
    exit 4
  fi
...
for RANK in 0 1 2 3; do
  ...
  TAYLORSEER_INVOCATION_ID="$START_NS" CUDA_VISIBLE_DEVICES="${GPU_IDS[$RANK]}" \
