...
class BenchmarkSpec:
    ...
    taylorseer: Callable[[], torch.Tensor]
    batch_size: int
...
def _measure(
    ...
    *,
    ...
    stats = getattr(function, "taylorseer_summary", None)
    if callable(stats):
        report.update(stats())
    return report
...
def benchmark_pair(
    ...
    *,
    ...
    """Measure matched Full then TaylorSeer under one factory-defined setup."""

    if os.environ.get("TAYLORSEER_GPU_TESTS_ALLOWED") != "1":
        raise RuntimeError("CUDA benchmark requires TAYLORSEER_GPU_TESTS_ALLOWED=1")
    ...
    median_speedup = (
        full["median_ms_per_image"] / taylorseer["median_ms_per_image"]
    )
    mean_speedup = full["mean_ms_per_image"] / taylorseer["mean_ms_per_image"]
    allocated_delta = int(taylorseer["peak_memory_allocated"]) - int(
        full["peak_memory_allocated"]
    )
    reserved_delta = int(taylorseer["peak_memory_reserved"]) - int(
        full["peak_memory_reserved"]
    )
    taylorseer["delta_memory_vs_full"] = allocated_delta
    taylorseer["delta_reserved_memory_vs_full"] = reserved_delta
    return {
        "protocol": {
