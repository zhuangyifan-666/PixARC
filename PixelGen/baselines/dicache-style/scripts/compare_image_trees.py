...
def main() -> None:
    ...
    reference_dir = Path(arguments.reference_dir).resolve(strict=True)
    candidate_dir = Path(arguments.candidate_dir).resolve(strict=True)
    sample_ids = numeric_image_ids(reference_dir)
    if sample_ids != numeric_image_ids(candidate_dir):
    ...
    report = {
        ...
        "exact": differing == 0,
        "limitation": "postprocessed PNG parity; tensor-level parity is a separate hard gate",
    }
    atomic_write_json(arguments.output_json, report)
    print(json.dumps(report, indent=2, sort_keys=True))
    if arguments.require_exact and differing:
