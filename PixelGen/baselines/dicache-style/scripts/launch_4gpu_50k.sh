...
usage() {
  echo "Usage: $0 --config FILE --manifest FILE --output-root DIR --release-gate FILE [--resume] [--nonfinal-proxy --expected-records N --expected-per-class N --expected-num-classes N]" >&2
}
...
while (($#)); do
  case "$1" in
    ...
    --release-gate) [[ $# -ge 2 ]] || { usage; exit 2; }; RELEASE_GATE="$2"; shift 2 ;;
...
if [[ $NONFINAL_PROXY -eq 0 ]]; then
  [[ -f "$RELEASE_GATE" ]] || {
    echo "Refusing final 50K launch without --release-gate FILE." >&2
    exit 2
  }
  ...
  if [[ -n "$RELEASE_GATE" && ! -f "$RELEASE_GATE" ]]; then
    echo "Refusing proxy launch: supplied release gate does not exist." >&2
    exit 2
  fi
...
RELEASE_GATE_SCRIPT="$SCRIPT_ROOT/release_gate.py"
for REQUIRED in "$GENERATE_SHARD" "$SNAPSHOT_SCRIPT" "$WALL_CLOCK_SCRIPT" "$GUARD_SCRIPT" "$RELEASE_GATE_SCRIPT"; do
...
if [[ -n "$RELEASE_GATE" ]]; then
  RELEASE_GATE="$(cd -- "$(dirname -- "$RELEASE_GATE")" && pwd -P)/$(basename -- "$RELEASE_GATE")"
  CUDA_VISIBLE_DEVICES='' python "$RELEASE_GATE_SCRIPT" verify \
    --model-family "$MODEL_FAMILY" --gate "$RELEASE_GATE" \
...
python "$GUARD_SCRIPT" --config "$CONFIG" --manifest "$MANIFEST" \
  ...
  --allowed-mode upstream_full --allowed-mode instrumented_full --allowed-mode dicache

if [[ "${DICACHE_GPU_TESTS_ALLOWED:-0}" != "1" ]]; then
  echo "Refusing GPU generation. Set DICACHE_GPU_TESTS_ALLOWED=1 only after all target GPUs are idle." >&2
  exit 2
fi
...
for DEVICE in "${GPU_IDS[@]}"; do
  ...
  for EXISTING_UUID in "${PHYSICAL_GPU_UUIDS[@]}"; do
    if [[ "$DEVICE_UUID" == "$EXISTING_UUID" ]]; then
      echo "Refusing launch: CUDA_VISIBLE_DEVICES aliases one physical GPU twice." >&2
      exit 3
    fi
  done
...
LOCK_DIR="$OUTPUT_ROOT/.dicache-launch.lock"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "Refusing launch: another DiCache launcher owns $LOCK_DIR." >&2
  exit 4
fi
...
GPU_LOCK_ROOT="/tmp/pixarc-dicache-gpu-locks"
...
for DEVICE_UUID in "${PHYSICAL_GPU_UUIDS[@]}"; do
  ...
  if ! mkdir "$GPU_LOCK_DIR" 2>/dev/null; then
    ...
    echo "Verify no DiCache rank is active before removing a stale lock." >&2
    exit 4
  fi
...
ARCHIVED_RELEASE_GATE="$OUTPUT_ROOT/release_gate.json"
archive_set_complete() {
  [[ -f "$ARCHIVED_CONFIG" && -f "$ARCHIVED_MANIFEST" && -f "$ARCHIVED_MANIFEST_SIDECAR" ]] || return 1
  if [[ $NONFINAL_PROXY -eq 0 ]]; then
    [[ -f "$ARCHIVED_RELEASE_GATE" ]] || return 1
  fi
}
...
if [[ $RESUME -eq 1 ]]; then
  ...
  if [[ $HAS_DURABLE_OUTPUT -eq 1 ]] \
    && ! archive_set_complete; then
    echo "Refusing resume: durable output exists but an archived config, manifest, sidecar, or final release gate is missing." >&2
    exit 5
  fi
  ...
  if [[ $NONFINAL_PROXY -eq 0 ]]; then
    [[ -f "$ARCHIVED_RELEASE_GATE" ]] || python "$SNAPSHOT_SCRIPT" "$RELEASE_GATE" "$ARCHIVED_RELEASE_GATE"
  fi
else
  ...
  if [[ $NONFINAL_PROXY -eq 0 ]]; then
    python "$SNAPSHOT_SCRIPT" "$RELEASE_GATE" "$ARCHIVED_RELEASE_GATE"
  fi
fi
...
if [[ $NONFINAL_PROXY -eq 0 ]]; then
  cmp -s "$RELEASE_GATE" "$ARCHIVED_RELEASE_GATE" || {
    echo "Refusing launch: release gate differs from archived release_gate.json." >&2
    exit 5
  }
fi
...
WORKER_RELEASE_ARGS=()
if [[ $NONFINAL_PROXY -eq 0 ]]; then
  # Revalidate the exact durable archive, then bind every worker to these bytes.
  CUDA_VISIBLE_DEVICES='' python "$RELEASE_GATE_SCRIPT" verify \
    --model-family "$MODEL_FAMILY" --gate "$ARCHIVED_RELEASE_GATE" \
    --config "$ARCHIVED_CONFIG" --manifest "$ARCHIVED_MANIFEST" >/dev/null
  ARCHIVED_RELEASE_GATE_SHA256="$(CUDA_VISIBLE_DEVICES='' python -c 'import hashlib,sys; print(hashlib.sha256(open(sys.argv[1], "rb").read()).hexdigest())' "$ARCHIVED_RELEASE_GATE")"
  WORKER_RELEASE_ARGS+=(
    --release-gate "$ARCHIVED_RELEASE_GATE"
    --release-gate-sha256 "$ARCHIVED_RELEASE_GATE_SHA256"
  )
else
  WORKER_RELEASE_ARGS+=(--nonfinal-proxy)
fi
...
for RANK in 0 1 2 3; do
  ...
  DICACHE_INVOCATION_ID="$START_NS" CUDA_VISIBLE_DEVICES="${GPU_IDS[$RANK]}" \
