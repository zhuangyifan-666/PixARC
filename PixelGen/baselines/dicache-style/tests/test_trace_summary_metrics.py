...
def test_trace_aggregate_exposes_scheduler_timing_and_memory_metrics():
    row = {
        "trajectory_id": "t",
        "direct_full_count": 2,
...
def test_full_trace_records_refresh_gap_and_summary_p95():
    ...
    assert [event["refresh_gap"] for event in summary["stream_trace"]] == [None, 1, 1]
    assert summary["mean_refresh_gap"] == 1.0
    assert summary["p95_refresh_gap"] == 1.0
    assert summary["max_refresh_gap"] == 1
