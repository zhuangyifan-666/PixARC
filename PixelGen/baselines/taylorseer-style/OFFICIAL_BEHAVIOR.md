...
for this **unofficial TaylorSeer-style port for PixelGen**. The paper and
...
PixelGen-XL has 28 blocks. One exact-Heun model evaluation concatenates
`[x,x]` with `[unconditional,conditional]` and makes one 2B forward, so all
blocks use one `combined_cfg` history. This is intentionally different from
JiT's two-stream adapter. Diagnostic `return_layer`/`return_last` requests
force that NFE Full. See `HEUN_ADAPTATION.md` for the 99-NFE mapping.
