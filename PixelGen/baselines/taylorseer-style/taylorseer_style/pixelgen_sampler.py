...
def _runtime_for(net: Any):
    return getattr(net, "taylor_runtime", None)


class TaylorSeerHeunSamplerJiT(HeunSamplerJiT):
    """Heun sampler assigning one Taylor decision to each actual 2B forward.
    ...
    def set_taylorseer_batch_context(
        self,
        *,
    ...
    def clear_taylorseer_batch_context(self) -> None:
        object.__setattr__(self, "_taylorseer_batch_context", None)

    @property
    def last_taylorseer_summary(self) -> dict[str, object] | None:
        return getattr(self, "_last_taylorseer_summary", None)

    def _next_trajectory_id(self) -> str:
        counter = int(getattr(self, "_taylorseer_trajectory_counter", 0))
        object.__setattr__(self, "_taylorseer_trajectory_counter", counter + 1)
    ...
    def _combined_forward(
        ...
        *,
        ...
        if runtime is None:
            raise RuntimeError("TaylorSeer sampler received a model without a runtime")
        runtime.begin_nfe(
        ...
        forward_taylor = getattr(net, "forward_taylor", None)
        if forward_taylor is None:
            raise RuntimeError("TaylorSeer model does not provide forward_taylor")
        out = forward_taylor(
            cfg_x,
            cfg_t,
            cfg_condition,
    ...
    def _impl_sampling(self, net, noise, condition, uncondition):
        ...
        try:
            if pending is None or pending.get("sample_ids") is None:
                raise ValueError(
                    "TaylorSeer PixelGen sampling requires manifest-stable sample_ids; "
                    "call set_taylorseer_batch_context before the batch"
                )
            batch_size = int(noise.shape[0])
            sample_ids = combined_cfg_sample_ids(
