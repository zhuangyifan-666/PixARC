...
if [[ "${TAYLORSEER_GPU_TESTS_ALLOWED:-0}" != "1" ]]; then
  echo "Deferred GPU smoke tests are locked. Set TAYLORSEER_GPU_TESTS_ALLOWED=1 only after the target GPU is idle." >&2
  exit 2
fi
...
TAYLORSEER_INVOCATION_ID="smoke-full-$(date +%s%N)" \
...
TAYLORSEER_INVOCATION_ID="smoke-candidate-$(date +%s%N)" \
python "$SCRIPT_ROOT/generate_shard.py" \
