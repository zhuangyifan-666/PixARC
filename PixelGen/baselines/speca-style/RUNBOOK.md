...
BATCH_SIZE=4
SMOKE_MANIFEST="$OUTPUT_ROOT/manifests/pixelgen_smoke8_b4.jsonl"
PROXY_MANIFEST="$OUTPUT_ROOT/manifests/pixelgen_imagenet1k_b4.jsonl"
VALIDATION_MANIFEST="$OUTPUT_ROOT/manifests/pixelgen_imagenet8k_b4.jsonl"
MANIFEST="$OUTPUT_ROOT/manifests/pixelgen_imagenet50k_b4.jsonl"
...
"$PIXELGEN_PYTHON" scripts/build_manifest.py --output "$SMOKE_MANIFEST" \
  ...
  --split-name smoke8 --world-size 1 --batch-size 4 \
...
Validate sidecars, class balance, fixed batch-4 groups, seeds, shards, and disjointness:
...
"$PIXELGEN_PYTHON" scripts/validate_manifest.py --manifest "$SMOKE_MANIFEST" \
  ...
  --world-size 1 --batch-size 4 --base-seed "$SMOKE_BASE_SEED" \
...
"$PIXELGEN_PYTHON" scripts/estimate_cache_memory.py --preset pixelgen-xl-256 \
  ...
  --output-json "$OUTPUT_ROOT/memory/pixelgen_b4_k${MAX_ORDER}.json"
```
...
The candidate must retain released scheduler, relative L1/`1e-10`, first-enhance 3, floor 0.01, last-block/all-token verification, batch-global gate, NFE coordinate, `interval: null`, no forced final Full, inherited cache dtype, real batch 4/effective batch 8, matched compile, exact Heun, timeshift/guidance, and `[unconditional, conditional]` order. Threshold selection is performed under this grouped-batch protocol.
...
Require checkpoint/`ema_denoiser`, finite output, both actions/checks, 99 combined forwards, effective batch 8 in correct order, exact diagnostics, reset, and no NaN.
...
```bash
RUNNER_CONFIG="$OUTPUT_ROOT/benchmark/pixelgen_runner_b4.json"
...
bash scripts/benchmark_single_gpu.sh \
  ...
  --output-json "$OUTPUT_ROOT/benchmark/pixelgen_b4_matched.json" \
...
Generate a new manifest-matched real-batch-4/effective-8 Full; do not pair against legacy outputs without immutable noise evidence:
...
```bash
FINAL_FULL_ROOT="$OUTPUT_ROOT/final/pixelgen_instrumented_full_b4"
FINAL_SPECA_ROOT="$OUTPUT_ROOT/final/pixelgen_speca_b4"
...
Only the new manifest-backed batch-4 matched Full is eligible:
