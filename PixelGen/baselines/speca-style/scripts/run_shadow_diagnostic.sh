...
if [[ "${SPECA_GPU_TESTS_ALLOWED:-0}" != "1" ]]; then
  echo "Shadow diagnostics are locked. Set SPECA_GPU_TESTS_ALLOWED=1 only after the target GPU is idle." >&2
  exit 2
fi
...
python "$SCRIPT_ROOT/deferred_run_guard.py" \
  ...
  --require-mode shadow_verify --require-trace-mode shadow
SPECA_INVOCATION_ID="shadow-$(date +%s%N)" \
