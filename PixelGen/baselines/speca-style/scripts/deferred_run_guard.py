...
from speca_style.manifest import load_manifest, validate_manifest  # noqa: E402
from speca_style.metadata import validate_speca_config  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="permit only these speca.mode values (repeatable)",
    )
    ...
    if config.get("schema_version") != "pixarc-speca-config-v1":
        raise ValueError("generation config must use pixarc-speca-config-v1")
    if not isinstance(config.get("speca"), dict):
        raise ValueError("generation config must contain one speca mapping")
    speca = validate_speca_config(config["speca"], require_resolved=True)
    if arguments.require_mode is not None and speca["mode"] != arguments.require_mode:
        raise ValueError(
            f"config speca.mode={speca['mode']!r}, expected {arguments.require_mode!r}"
        )
    if arguments.allowed_mode and speca["mode"] not in set(arguments.allowed_mode):
        raise ValueError(
            f"config speca.mode={speca['mode']!r} is not allowed here; "
            f"expected one of {sorted(set(arguments.allowed_mode))}"
        )
    if (
        arguments.require_trace_mode is not None
        and speca["trace_mode"] != arguments.require_trace_mode
    ):
        raise ValueError(
            "config speca.trace_mode="
            f"{speca['trace_mode']!r}, expected {arguments.require_trace_mode!r}"
        )
    ...
    if batch_size != 1 or effective_cfg_batch_size != 2:
        raise ValueError(
            "primary PixelGen SpeCa runs require runtime.batch_size=1 and "
    ...
    print(
        ...
        f"{len(records)} records, mode={speca['mode']}, batch_size={batch_size}"
    )
