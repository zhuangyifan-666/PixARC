...
def test_only_resolved_layer_verifies_when_check_is_true():
    ...
    assert runtime.should_verify(layer_idx=27, depth=28)
    assert not runtime.should_verify(layer_idx=26, depth=28)
    runtime.reset()
...
def test_check_false_disables_verification_even_at_last_layer():
    ...
    assert not runtime.should_verify(layer_idx=27, depth=28)
    runtime.reset()
