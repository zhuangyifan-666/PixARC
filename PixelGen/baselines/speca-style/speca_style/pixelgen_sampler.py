...
def _runtime_for(net: Any):
    return getattr(net, "speca_runtime", getattr(net, "taylor_runtime", None))


class SpeCaHeunSamplerJiT(HeunSamplerJiT):
    """Heun sampler assigning one SpeCa decision to each actual 2B forward.
    ...
    def clear_speca_batch_context(self) -> None:
        object.__setattr__(self, "_speca_batch_context", None)

    @property
    def last_speca_summary(self) -> dict[str, object] | None:
        return getattr(self, "_last_speca_summary", None)

    def _next_trajectory_id(self) -> str:
        counter = int(getattr(self, "_speca_trajectory_counter", 0))
        object.__setattr__(self, "_speca_trajectory_counter", counter + 1)
    ...
    def _combined_forward(
        ...
        *,
        ...
        if runtime is None:
            raise RuntimeError("SpeCa sampler received a model without a runtime")
        runtime.begin_nfe(
        ...
        forward_speca = getattr(net, "forward_speca", None)
        if forward_speca is None:
            raise RuntimeError("SpeCa model does not provide forward_speca")
        out = forward_speca(
            cfg_x,
            cfg_t,
            cfg_condition,
    ...
    def _impl_sampling(self, net, noise, condition, uncondition):
        runtime = _runtime_for(net)
        pending = getattr(self, "_speca_batch_context", None)
        ...
        if runtime is None or runtime.mode == "upstream_full":
            try:
                ...
                self.clear_speca_batch_context()
        ...
        try:
            if pending is None or pending.get("sample_ids") is None:
                raise ValueError(
                    "SpeCa PixelGen sampling requires manifest-stable sample_ids; "
                    "call set_speca_batch_context before the batch"
                )
            ...
            runtime.begin_trajectory(
                ...
                trajectory_id=pending.get("trajectory_id")
                or self._next_trajectory_id(),
