"""Minimal PixelGen Lightning integration for batch-scoped SpeCa histories."""
...
class SpeCaPixelGenLightning(_UpstreamLightningModel):
    ...
    def __init__(
        ...
        object.__setattr__(self, "_speca_compile_mode", compile_mode)
        ...
        source_runtime = getattr(self.denoiser, "speca_runtime", None)
        ema_runtime = getattr(self.ema_denoiser, "speca_runtime", None)
        if source_runtime is None or ema_runtime is None:
            raise TypeError("both PixelGen denoisers must expose speca_runtime")
        if source_runtime is ema_runtime:
            raise RuntimeError("denoiser and EMA unexpectedly share SpeCa runtime")
        if ema_runtime.active or ema_runtime.tensor_count() != 0:
            raise RuntimeError("deepcopied EMA SpeCa runtime must start empty")

    @property
    def speca_compile_mode(self) -> str:
        return self._speca_compile_mode
    ...
    def configure_model(self) -> None:
        ...
        for name, model in (
            ...
            if getattr(model, "compile_mode", None) != self._speca_compile_mode:
    ...
    def predict_step(self, batch, batch_idx):
        ...
        runtime = getattr(net, "speca_runtime", None)
        setter = getattr(
            self.diffusion_sampler, "set_speca_batch_context", None
        )
        clearer = getattr(
            self.diffusion_sampler, "clear_speca_batch_context", None
        )
        if runtime is None or runtime.mode == "upstream_full":
            return super().predict_step(batch, batch_idx)
        if setter is None:
            raise TypeError(
                "SpeCa PixelGen model requires SpeCaHeunSamplerJiT"
            )
