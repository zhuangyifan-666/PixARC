...
from .metadata import SPECA_CONFIG_FIELDS, atomic_write_json
...
class ManifestNoiseDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        batch_size: int,
        config_hash: str,
        speca_config_hash: str,
        checkpoint_path: str,
        checkpoint_size: int,
        method: str,
        speca_config: Mapping[str, Any],
        resolution: int = 256,
        ...
        missing = [field for field in SPECA_CONFIG_FIELDS if field not in speca_config]
        if missing:
            raise ValueError(f"speca_config is missing fields: {missing}")
        method_config = {field: speca_config[field] for field in SPECA_CONFIG_FIELDS}
        ...
        self.speca_config_hash = str(speca_config_hash)
        self.checkpoint_path = str(checkpoint_path)
        self.checkpoint_size = int(checkpoint_size)
        self.method = str(method)
        self.speca_config = method_config
    ...
    def __getitem__(self, index: int):
        ...
        metadata = {
            ...
            "speca_config_hash": self.speca_config_hash,
            "checkpoint_path": self.checkpoint_path,
            "checkpoint_size": self.checkpoint_size,
            "real_batch_size": self.real_batch_size,
            "effective_cfg_batch_size": self.effective_cfg_batch_size,
        }
        metadata.update({"method": self.method, **self.speca_config})
...
class AtomicManifestSaveHook(Callback):
    ...
    def __init__(
        ...
        *,
        ...
        self.invocation_id = os.environ.get("SPECA_INVOCATION_ID")
    ...
    def on_predict_batch_end(
        ...
        summary = getattr(pl_module.diffusion_sampler, "last_speca_summary", None)
        if not summary:
