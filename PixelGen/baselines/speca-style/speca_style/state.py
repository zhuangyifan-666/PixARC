...
@dataclass
class SpeCaSchedulerState:
    last_type: str = "None"
    last_verification_error: float | None = 0.0
    taylor_step_counter: int = 0
    cache_counter: int = 0
    check: bool = False
...
@dataclass
class SpeCaTrajectoryState:
    trajectory_id: str | None
    sample_ids: list[int]
    total_nfe: int
    nfe_index: int = 0
    q: int | None = None
    solver_stage: str | None = None
    scheduler: SpeCaSchedulerState = field(default_factory=SpeCaSchedulerState)
    streams: dict[Hashable, TaylorStreamState] = field(default_factory=dict)
